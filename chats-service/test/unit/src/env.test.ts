...
describe('env', () => {
  beforeAll(() => {
    ...
    process.env = {
      ...
      APP_HOST: 'localhost',
      APP_SCHEMA: 'http',
      APP_ROUTE_PREFIX: '/chats-service/api',
      PORT: '3002',
      APP_BANNER: 'true',
      LOG_LEVEL: 'info',
      LOG_JSON: 'false',
      ...
      ERROR_CODE_PREFIX: 'CHATS_SERVICE',
  ...
  it.skip('should load the correct .env file', () => {
    expect(mockDotenvConfig).toHaveBeenCalledWith({
      path: process.cwd() + `/.env${process.env.NODE_ENV === 'test' ? '.test' : ''}`
    });
  });
  ...
  it('should have the correct app name', () => {
    expect(env.app.name).toEqual(process.env.APP_NAME || 'chat-service');
  });
  ...
  it('should have the correct app routePrefix', () => {
    expect(env.app.routePrefix).toEqual(process.env.APP_ROUTE_PREFIX || '/chats-service/api');
  });
  ...
  it('should have the correct mongoURL', () => {
    expect(env.db.mongoURL).toEqual(process.env.MONGO_URL || 'mongodb://127.0.0.1:27017/ChatDB');
  });
  ...
  it('should have the correct error prefix', () => {
    expect(env.errors.errorPrefix).toEqual(process.env.ERROR_CODE_PREFIX || 'CHATS_SERVICE');
  });
