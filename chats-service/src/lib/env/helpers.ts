...
export const decryptValue = (encryptedText: string): string => {
  try {
    ...
    console.error('Decryption failed for text:', encryptedText.substring(0, 20) + '...', 'Error:', error.message);
    // Return the encrypted text if decryption fails to avoid breaking the app
    return encryptedText;
  }
};
